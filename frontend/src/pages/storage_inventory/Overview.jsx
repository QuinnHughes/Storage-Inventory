    ...
    <div className="max-w-5xl">
      ...
      {!loading && data && (
        <>
                      ...
                      >
                        <td className="py-3 pr-4">
                          ...
                          {loc.code !== "__uncategorized__" && (
                            <div className="text-xs text-gray-400 mt-0.5">{loc.display_name}</div>
                          )}
                        </td>
                        <td className="py-3 px-4 text-right tabular-nums">
                          ...
                          <span className="text-gray-400 text-xs ml-1">{loc.coverage_pct}%</span>
                        </td>
                        <td className="py-3 px-4 text-right tabular-nums">
                          {loc.by_severity.error > 0 ? (
                            <span className="text-red-600 font-medium">{loc.by_severity.error}</span>
                          ) : (
                            <span className="text-gray-300">—</span>
                          )}
                        </td>
                        <td className="py-3 px-4 text-right tabular-nums">
                          {loc.by_severity.warning > 0 ? (
                            <span className="text-amber-600 font-medium">{loc.by_severity.warning}</span>
                          ) : (
                            <span className="text-gray-300">—</span>
                          )}
                        </td>
                        <td className="py-3 px-4 text-right tabular-nums text-gray-600">
                          {loc.discrepancies_total > 0 ? (
                            <>
                              ...
                              <span className="text-gray-400 text-xs ml-1">{loc.resolution_pct}%</span>
                            </>
                          ) : (
                            <span className="text-gray-300">—</span>
                          )}
                        ...
                        <td className="py-3 px-4 text-right tabular-nums text-gray-600">
                          {loc.inches_measured > 0 ? `${loc.inches_measured}"` : <span className="text-gray-300">—</span>}
                        </td>
                        <td className="py-3 pl-4 text-right">
                          {loc.last_inventoried ? (
                            <div>
                              <div className="text-gray-700">{formatDate(loc.last_inventoried)}</div>
                              <div className="text-xs text-gray-400">{timeAgo(loc.last_inventoried)}</div>
                            </div>
                          ) : (
          ...
          {activeStats.discrepancies_total === 0 && activeStats.shelves_total === 0 && (
            <div className="mt-6 bg-white rounded-xl border border-gray-200 shadow-sm px-6 py-12 text-center">
              <p className="text-gray-400 text-sm">
                No mapping data or scan sessions found for Storage.
              </p>
...
function StatCard({ label, value, sub, color }) {
  const valueColors = {
    ...
    amber: "text-amber-600", blue: "text-blue-600", gray: "text-gray-700",
  };
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm px-5 py-4">
      <div className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-1">{label}</div>
      <div className={`text-2xl font-bold leading-tight ${valueColors[color] ?? "text-gray-800"}`}>{value}</div>
...
function SevDot({ sev }) {
  const colors = { error: "bg-red-500", warning: "bg-amber-400", info: "bg-blue-400" };
  return <span className={`inline-block w-2 h-2 rounded-full flex-shrink-0 ${colors[sev] ?? "bg-gray-300"}`} />;
}
